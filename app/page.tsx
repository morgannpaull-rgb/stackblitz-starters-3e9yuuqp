...
const DEFAULT_GIVEBACK_THRESHOLD = 0.30; // per request, July 26 — Trail-Stop (renamed from "Gains Giveback"): session ends if THIS account's ROI has dropped this many FIXED percentage-points off its own peak — e.g. peak +40% at 30 points → triggers once ROI drops to +10%, same fixed gap regardless of how big the peak is. The trigger level only ever moves up as new peaks form. Now user-adjustable in Settings.
...
function settleSpin(history: Step[], outcome: SpinValue, baseUnit: number, startingBankroll: number, strategy: Strategy, pulseEnabled: boolean, bbStraightEnabled: boolean, bbInvertedEnabled: boolean, executionMode: ExecutionMode = "Stream Direct", tableLimit = DEFAULT_TABLE_LIMIT, perNumberLimit = DEFAULT_PER_NUMBER_LIMIT, tierExecution: TierExecutionSettings = DEFAULT_TIER_EXECUTION, markovEnabled = false, randomEnabled = false, stopLossThreshold = DEFAULT_STOP_LOSS_THRESHOLD, givebackThreshold = DEFAULT_GIVEBACK_THRESHOLD): Step {
  ...
  //   2. TRAIL-STOP (gain-side, renamed from "Gains Giveback" per request,
  //      July 26): this account's ROI has dropped a fixed number of
  //      percentage-points off its own peak, regardless of how big that
  //      peak was — the traditional, fixed-distance version of a trailing
  //      stop, not scaled to peak size. The trigger level only ever moves
  //      UP as new peaks form; a temporary dip never pulls it back down.
  ...
  if (pulseEnabled && !alreadyEnded) {
    ...
    const trailStopHit = peakRoi > 0 && (peakRoi - curRoi) >= givebackThreshold;
  ...
  return {
    ...
    note: autoStopTriggered
      ? (alreadyEnded
          ...
          ? `SESSION ENDED — Trail-Stop hit (ROI dropped ${Math.round(givebackThreshold * 100)} points off its peak, locking in the gain)`
